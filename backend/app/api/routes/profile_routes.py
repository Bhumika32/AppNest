...
def get_dashboard_summary():
    ...
    try:
        ...
        active_quests = QuestService.get_active_quests(user_id)
        ...
        for uq in active_quests:
            formatted_quests.append({
                ...
                "color": "neon-blue" # Default UI color
            })

        # Fill with mocks if empty to keep UI looking good but functional
...
def submit_feedback():
    ...
    if not message or rating is None:
        return jsonify(error="Message and rating are required"), 400

    user_id = int(get_jwt_identity())
    try:
        UserProfileService.submit_feedback(user_id, message, rating)
        return jsonify(success=True, message="Feedback recorded"), 201
    except Exception as e:
        return jsonify(error=str(e)), 500
