...
try:
    ...
    if user_role != "ADMIN":
        print(f"   ⚠️  WARNING: Expected 'ADMIN', got '{user_role}'")
...
print(f"\n2️⃣  Calling /api/auth/me with access token...")
try:
    ...
    if me_role != "ADMIN":
        print(f"   ⚠️  WARNING: Expected 'ADMIN', got '{me_role}'")
...
print(f"\n3️⃣  Checking HttpOnly refresh token cookie...")
try:
    cookies = session.cookies.get_dict()
    if "refresh_token" in cookies:
        print(f"   ✓ refresh_token cookie present (HttpOnly, value hidden)")
    else:
        print(f"   ⚠️  refresh_token cookie NOT found")
...
print("\n" + "=" * 70)
print("✅ Backend auth flow test complete!")
print("=" * 70)
print("\nTroubleshooting:")
