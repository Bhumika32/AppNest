"""
app/tools/age/service.py

Age calculation logic for AppNest.

Goals:
- Accurate age calculations
- Fun + Roast modes for youth engagement
- Professional structure (service + dataclass result)
- No offensive / unsafe content (roast = playful only)
...
- Total weeks lived
- Total hours lived
- Days until next birthday + next birthday date
- Born day (weekday)
- Sun Sign (Western zodiac)
- Messaging engine:
    * Normal Mode -> professional
    * Fun Mode    -> funny
    * Roast Mode  -> spicy + personal but safe
"""
...
@dataclass
class AgeResult:
    """Structured output returned by AgeService."""
...
class AgeService:
    """Service class for age calculations + message generation."""
    ...
    def _zodiac_sign(dob: date) -> str:
        """Return Sun Sign (Western) based on DOB."""
        day = dob.day
        month = dob.month
    ...
    def _birthday_hype(days_until_birthday: int) -> str:
        """Extra excitement near birthdays."""
    ...
    @staticmethod
    def calculate(name: str, dob: date, fun_mode: bool = True, roast_mode: bool = False) -> AgeResult:
        """Main entry point for age calculation."""
        today = date.today()
