/**
 * FILE: frontend/src/modules/games/FlappyBirdGame.jsx
 *
 * Flappy Bird mini-game module used inside the AppNest Game Engine.
 *
 * Responsibilities:
 * - Handles bird physics (gravity, jumping)
 * - Spawns and moves obstacles
 * - Detects collisions
 * - Calculates score
 * - Sends result to engine when game ends
 * - Triggers roast / mentor events
 *
 * This component is optimized to avoid React render loops
 * by using requestAnimationFrame with controlled dependencies.
 */
...
const FlappyBirdGame = ({ engine }) => {
  ...
  /**
   * Mutable state container for values that should not trigger re-renders.
   * Used for:
   * - crash detection
   * - roast trigger control
   */
  const state = useRef({
    didCrash: false,
    pendingRoast: false,
    ...
    const updateGame = () => {
      ...
      setBirdY((prev) => {
        ...
        if (newY >= GAME_HEIGHT || newY <= 0) {
          return newY >= GAME_HEIGHT ? GAME_HEIGHT : 0;
        }
  ...
  }, [isPlaying, isGameOver, velocity, birdY, generateObstacle]);
  ...
  useEffect(() => {
    if (!isGameOver && (birdY >= GAME_HEIGHT || birdY <= 0 || state.current.didCrash)) {
      handleGameOver();
    }
      ...
      >
        ...
        {!isPlaying && !isGameOver && (
          <div className="absolute inset-0 flex items-center justify-center bg-black/50 backdrop-blur-sm">
            <span className="text-white font-black uppercase tracking-widest animate-pulse">
        ...
        {isGameOver && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-black/80 backdrop-blur-md">
            <span className="text-4xl font-black text-red-500 uppercase tracking-widest mb-2 shadow-red-500 drop-shadow-lg">
              Crashed
            </span>
